...
class StubAgentService:
    ...
    AgentService.answer(), without depending on a real Retriever/
    LLMProvider/ToolRegistry (already covered by Step 11/15's own tests).
    """
...
def test_new_chat_without_conversation_id_creates_a_conversation():
    service = ChatService(StubAgentService(), InMemoryConversationStore(), history_window=6)
...
def test_followup_with_conversation_id_reuses_the_same_conversation():
    ...
    service = ChatService(StubAgentService(), store, history_window=6)
...
def test_conversation_is_updated_with_user_message_and_assistant_response():
    store = InMemoryConversationStore()
    service = ChatService(StubAgentService(reply="the answer"), store, history_window=6)
...
def test_followup_request_passes_previous_turns_as_history_to_answer_generator():
    store = InMemoryConversationStore()
    generator = StubAgentService(reply="first answer")
